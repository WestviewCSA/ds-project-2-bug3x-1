...
public class p2 {
        ...
        for (int[] dir : directions) {
            ...
            if (neighbor != null && !visited.contains(neighbor) && neighbor.getCharacter() != '@') {
                if (neighbor.getCharacter() == '.') {
                    neighbor.setCharacter('+'); // Change path to '+'
                }
                queue.add(neighbor);
    ...
    public static void printMap(Tile[][][] tileMap) {
        for (int room = 0; room < tileMap.length; room++) {
            System.out.println("Room " + (room + 1) + ":");
            for (int row = 0; row < tileMap[room].length; row++) {
                for (int col = 0; col < tileMap[room][row].length; col++) {
                    System.out.print(tileMap[room][row][col].getCharacter() + " ");
                }
                System.out.println();
            }
            System.out.println();
